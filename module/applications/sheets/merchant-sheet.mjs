...
export class MerchantSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
  ...
  #parsePriceValue(value) { return parsePriceValue(value) }
  #parseQuantityValue(value) { return parseQuantityValue(value) }
  #normalizeCurrencyKey(c) { return normalizeCurrencyKey(c) }
  #formatCurrencyLabel(c) { return formatCurrencyLabel(c) }
  #formatPriceLabel(v, c) { return formatPriceLabel(v, c) }
  #escapeHTML(v) { return escapeHTML(v) }
  #slugifyCategoryKey(v) { return slugifyCategoryKey(v) }
  ...
  #getModuleSetting(key) { return getModuleSetting(key) }
  #getConfiguredItemValue(item, key) { return getConfiguredItemValue(item, key) }
  #getAllowedTypes(key) { return getAllowedTypes(key) }
  #isItemTypeAllowed(item, key) { return isItemTypeAllowed(item, key) }
  #getCategoryPaths() { return getCategoryPaths() }
  #getCategoryLabelMap() { return getCategoryLabelMap() }

  // ─── Wrappers for imported catalog functions ──────────────────────────────

  #getSellPercent() { return getSellPercent(this.actor) }
  #adjustPriceValue(basePriceValue) { return adjustPriceValue(basePriceValue, this.#getSellPercent()) }
  #getReferenceCurrency() { return getReferenceCurrency() }
  #getItemAvailableQuantity(item) { return getItemAvailableQuantity(item) }
  #getItemCurrencyHelper(item) { return getItemCurrency(item) }

  #getAutomaticItemCategory(item) { return getAutomaticItemCategory(item) }
  ...
  #prepareSellerItemDropData(item) { return prepareSellerItemDropData(this.actor, item) }

  // ─── Wrappers for imported trade functions ────────────────────────────────

  #normalizeSession(session) { return normalizeSession(session) }
  #normalizeSessionItem(item) { return normalizeSessionItem(item) }
  #normalizeAccessClient(client) { return normalizeAccessClient(client) }
  #buildAccessClientFromActor(actor, opts) { return buildAccessClientFromActor(actor, opts) }
  #buildSessionData(client) { return buildSessionData(client) }
  #getSessions() { return getSessions(this.actor) }
  #getStoredAccessClients() { return getStoredAccessClients(this.actor) }
  #recalculateSessionItemTotal(item) { return recalculateSessionItemTotal(item) }
  #setSessionItemQuantity(item, quantity) { return setSessionItemQuantity(item, quantity) }
  ...
  #getConfiguredCurrency(currency) { return getConfiguredCurrency(currency) }
  #getMerchantWalletAmount(currency) { return getMerchantWalletAmount(this.actor, currency) }
